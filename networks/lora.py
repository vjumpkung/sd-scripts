...
class LoRAModule(torch.nn.Module):
    ...
    def validate_norm_approximation(self, org_module_weight: Tensor, verbose=True):
        ...
        for i in range(0, org_module_weight.shape[0], chunk_size):
            ...
            chunk = org_module_weight[i:end_idx].to(
    ...
    def prepare_optimizer_params_with_multiple_te_lrs(
        ...
        if self.text_encoder_loras:
            ...
            te1_loras = [
                ...
                if lora.lora_name.startswith(self.LORA_PREFIX_TEXT_ENCODER1)
            ]
            te2_loras = [
                lora
                for lora in self.text_encoder_loras
                if lora.lora_name.startswith(self.LORA_PREFIX_TEXT_ENCODER2)
            ]
            if len(te1_loras) > 0:
                logger.info(
